...
class KANStack(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim, kan_params):
        super().__init__()
        self.layer1 = KAN(
            width=[in_dim, hidden_dim],
            grid=kan_params["grid"],
            k=kan_params["k"],
            grid_range=kan_params["grid_range"],
        )
        self.layer2 = KAN(
            width=[hidden_dim, out_dim],
            grid=kan_params["grid"],
            k=kan_params["k"],
            grid_range=kan_params["grid_range"],
        )
...
class SeqKAN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, kan_params=None):
        ...
        self.kan_hidden = KANStack(
            in_dim=input_size + hidden_size,
            hidden_dim=hidden_size,
            out_dim=hidden_size,
            kan_params=kan_params["hidden"],
        )
        self.kan_out = KANStack(
            in_dim=hidden_size,
            hidden_dim=output_size,
            out_dim=output_size,
            kan_params=kan_params["output"],
        )
...
class TS_seqKAN(ODEJump):
    def __init__(
        ...
        self.seqkan = SeqKANCore(
            hidden_dim,
            input_dim=in_channels * 2 if self.direct_x else hidden_dim,
    ...
    def forward(
        ...
        if not already_latent:
            ...
            if self.direct_x:
                ...
                h_base = x_fused  # (B,T,in_channels)
            else:
        ...
        #h = h + tm_e
        if self.direct_x:
            h_in = torch.cat([h_base, mask], dim=-1)
            delta = self.seqkan(h_in)
            h = h_base + delta
        else:
            h = self.seqkan(h)
        state = h
...
class TSDF_seqKAN(TSDiffusion):
    def __init__(
        ...
        if self.feature_scale:
            # Learnable per-feature gain applied to inputs before KAN splines.
            # Use exp() to keep positive scaling; exp(0)=1 preserves scale initially.
        ...
        if status_dim > 0:
            ...
            self.encoder_ode_tmax = SeqKANEncoder(
                self.state_dim * 2 if self.direct_x else self.state_dim,
                self.state_dim,
                kan_params=kan_params,
                variational_dropout=variational_dropout,
            )
    ...
    def forward(
        ...
        if self.lam[0]>0 or self.lam[4]>0:
            if self.direct_x:
                h_in = torch.cat([h, mask], dim=-1)
                delta = self.encoder_ode_x(h_in, timestamps, only_gru)
                h = h + delta
            else:
                h = self.encoder_ode_x(h, timestamps, only_gru)
        if self.lam[2]>0 or self.lam[5]>0:
            if self.direct_x:
                ht_in = torch.cat([h, mask], dim=-1)
                ht = self.encoder_ode_tmax(ht_in, timestamps, only_gru)
            else:
